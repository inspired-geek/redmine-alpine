...
plugin assets, application assets, themes, and database data, and waits for a
patch-pinned MariaDB image to become healthy.
...
The 7.0 Compose example additionally persists `/usr/src/redmine/public/assets`.
Named volumes inherit image ownership on first use. Bind-mounted directories
must be writable by UID 1001 or group 0; arbitrary UIDs in group 0 are also
supported.
